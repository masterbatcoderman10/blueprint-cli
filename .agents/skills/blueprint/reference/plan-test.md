---
name: plan-test
description: Create a test plan for a phase by mapping specific tests to tasks; routes test-planning intent
---
# Test Planning
...
<TestIDConvention>
  ...
  FORMAT:
    Single test for a task:   T-<task ID>        →  T-M2-4.0.1, T-M2-4.A.3
    Multiple tests for a task: T-<task ID>.<seq>  →  T-M2-4.A.2.1, T-M2-4.A.2.2

  Within a phase document's Test Plan section, the short form
  may be used (T-4.0.1, T-A.3) since the milestone-phase prefix
  is established in the document header.

  This makes it trivial to trace from a failing test back to the
  task it verifies, the stream it belongs to, and which milestone
  and phase produced it.
</TestIDConvention>
