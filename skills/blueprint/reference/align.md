...
<AlignmentFlow>
  ...
  STEP 1 -- ASSESS SETUP CONTEXT
    ...
    agent guidance
    CHECK -- codebase structure, tech stack, and testing
    infrastructure if present
    CHECK -- file organization, coding standards, release workflow,
    and project-specific constraints if present
  ...
  STEP 2 -- CONFIRM SETUP WITH USER
    Present a concise setup summary:
      ...
      - test infrastructure or the lack of it
      - file structure and coding standards
      - release notes or deployment constraints if found
      - agent workflow expectations and orchestration guidance if found

    Ask focused questions in small chunks. Let each answer close gaps
    in the setup guidance before proceeding.
