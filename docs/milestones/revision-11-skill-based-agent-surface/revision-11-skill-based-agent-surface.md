...
Distribution gains an **npx pathway** alongside the existing local-scaffold copy. The primary pathway uses [`vercel-labs/skills`](https://github.com/vercel-labs/skills), the community-standard skill-management CLI: users run `npx skills add masterbatcoderman10/blueprint-cli --skill blueprint`, which pulls the skill payload directly from this repo's public GitHub source and installs it project-locally into `.claude/skills/blueprint/` — the canonical path Claude Code discovers natively. No npm publication is strictly required for this pathway; the GitHub source is the install target. Phase 4 adds the repo-root `skills/blueprint/**` mirror and release verification needed for this install path. A first-party CLI install option is deferred to Revision 11 Phase 6 rather than introduced here.

For the vercel pathway to work, the skill payload must live at a path vercel-labs/skills knows how to scan in the repo (`skills/`, `.agents/skills/`, or repo root); we standardise on `skills/blueprint/` while keeping `templates/skills/blueprint/` authoritative for scaffold and Doctor surfaces. Global install (`npx skills add ... -g`) is a known sharp edge today — it lands in `~/.agents/skills/` rather than `~/.claude/skills/` (vercel-labs/skills issues [#693](https://github.com/vercel-labs/skills/issues/693), [#744](https://github.com/vercel-labs/skills/issues/744), [#851](https://github.com/vercel-labs/skills/issues/851)) — so the recommended install is project-local (default). We document this caveat rather than work around it.
...
- `package.json` — `files` array gains `skills/`
- `src/release/package-artifact.ts`, `src/release/verify-package-artifact.ts`, `docs/release-contract.md`, `docs/releasing.md` — verify `skills/` ships in tarball and document the primary `npx skills add ...` pathway
...
> The system must support a single primary NPX install pathway for the `blueprint` skill, targeting the canonical project-local `.claude/skills/blueprint/` directory via `vercel-labs/skills`.
>
...
> ```
>
> - `templates/skills/blueprint/**` remains the authoritative scaffold and Doctor source; repo-root `skills/blueprint/**` is a byte-identical mirror for external discovery.
> - The mirrored payload must include `SKILL.md`, all 20 renamed `reference/*.md` files, shared `reference/anti-patterns.md`, and `scripts/load-context.mjs`.
> - The release-artifact verifier must confirm `skills/blueprint/**` is present in the published tarball.
> - README and `docs/releasing.md` must document this command as the recommended project-local install path.
>
...
Add repo-root `skills/blueprint/**` as a byte-identical mirror of authoritative `templates/skills/blueprint/**` so `npx skills add masterbatcoderman10/blueprint-cli --skill blueprint` (vercel-labs/skills) works out of the box against the public GitHub repository. Add README install guidance documenting this command as the recommended project-local skill install path. Add `skills/` to the `package.json` `files` array and update `src/release/package-artifact.ts` + `verify-package-artifact.ts` to enforce the new payload. Update `docs/release-contract.md` and `docs/releasing.md` with the repo-root skill mirror contract, the project-local recommendation, and the `-g` global-install sharp-edge caveat. Real GitHub install verification is required as a manual smoke only; automated coverage remains local and deterministic. Create and activate MAS-210.
...
A. The npm tarball contains `skills/blueprint/SKILL.md`, all 20 `reference/*.md` files, shared `reference/anti-patterns.md`, and `scripts/load-context.mjs` — verified by `verify-package-artifact.ts`.
...
E. `npx skills add masterbatcoderman10/blueprint-cli --skill blueprint` (vercel-labs/skills) in a directory with no Blueprint footprint creates a valid project-local `.claude/skills/blueprint/` tree and exits 0 without modifying anything else. This GitHub-backed install is verified as a manual smoke. Automated coverage verifies the repo-root `skills/blueprint/**` mirror and packaged artifact locally. README documents the command, recommends project-local install, and surfaces the `-g` global-install sharp edge.
