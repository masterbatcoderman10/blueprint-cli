...
> Lock the bootstrap-state matrix, reserve the Foundation Planning skill surface, and update the shared canonical-path helpers before downstream mirrors, placeholder entry points, and tests are touched.
...
| R12-1.0.2 | Author `templates/skills/blueprint/reference/foundation-planning.md` and `skills/blueprint/reference/foundation-planning.md` as the locked Phase 1 bootstrap contract for the future workflow: preconditions (`alignment-complete`, empty progress, required setup blocks, tracker available), artifact order (`PRD Stage 1 -> SRS -> PRD Stage 2 -> project-progress`), one-artifact-at-a-time review rule, and explicit non-goals (no milestone docs, phase docs, test plans, or tracker tasks). The deeper interviewing and drafting workflow remains Phase 3 work. | 0.75 | R12-1.0.1 | Dependent |
