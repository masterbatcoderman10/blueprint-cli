...
| R7-1.B.4 | Update `docs/core/execution.md`, `docs/core/review.md`, and `docs/core/phase-completion.md`: allow tweak documents as execution and review sources alongside phase documents; phase-completion does not own standalone tweak completion (that lives in `tweak-planning.md`). Add an explicit tweak-completion gate in `execution.md`: the full project test suite (`npm test`) must pass before a tweak's terminal task is marked DONE; failure blocks completion. | 1.25 | Gate | Dependent |
...
- [ ] `execution.md` defines an explicit tweak-completion gate requiring `npm test` to pass before a tweak's terminal task moves to DONE.
