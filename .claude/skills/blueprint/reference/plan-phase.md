---
name: plan-phase
description: Plan and structure a phase document with gates, streams, and task tables; routes phase-planning intent
---
# Phase Planning
...
<TaskConventions>
  ...
  TASK ID FORMATS:
    Every task ID carries its milestone and phase context.

    Gate tasks:   M<milestone>-<phase>.<gate>.<seq>
                  →  M2-4.0.1, M2-4.0.2, M1-1.0.3
  ...
  DEPENDENCY NOTATION:
    ...
    A.2         — Depends on a specific task (same phase)
  ...
  TYPE:
    ...
    Dependent   — Blocked by one or more prior tasks. Cannot start
                  until all listed dependencies reach IN-REVIEW.

    The Type column is critical for parallel execution. Sub-agents
    use it to identify which tasks can run concurrently. Phase
    planning must set this column accurately — an incorrectly
    marked Independent task that actually depends on another
    will cause conflicts during parallel execution.
</TaskConventions>
